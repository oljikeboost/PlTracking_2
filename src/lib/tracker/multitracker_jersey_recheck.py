...
from trackr.matching import ious
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat, curr_jersey=None, buffer_size=90):
    ...
    def activate(self, kalman_filter, frame_id, jersey_num=None):
        ...
        self.jersey_list.append(jersey_num)

    def re_activate(self, new_track, frame_id, new_id=False, jersey_num=None):
    ...
    def update(self, new_track, frame_id, jersey_num=None, update_feature=True):
        """
...
class JDETracker(object):
    def __init__(self, opt,
        ...
        self.emb_assign = emb_assign
        self.iou_assign = iou_assign
        self.unconf_assign = unconf_assign
    ...
    def get_recheck_map(self, curr_raw_feature, prev_id_feature, orig_feat):
    ...
    def fuse_recheck(self, outputs, dets, eps=0.5):

        new_dets = []
        for b_i in outputs:
            s = 1 - max(ious(b_i, dets))
            if s>eps:
                new_dets.append(b_i)

        dets = dets.extend(new_dets)
        return dets
    ...
    def update(self, im_blob, img0):
        ...
        with torch.no_grad():
            output, orig_feat = self.model(im_blob)[-1]
            hm = output['hm'].sigmoid_()
            wh = output['wh']
            id_feature = output['id']
            id_feature = F.normalize(id_feature, dim=1)
            raw_id_feature = id_feature.copy()
        ...
        ### Here, we will pass the current and the previous features to the
        ### recheck network, and get the loss detections
        ### After that, we need to apply the fusion algorithm to detect the lost detections
        ### If there are some, append them to the 'dets'


        ### save the last feature as class attribute, so, we don't have to pass it to function
        ### also, the the frame number of the last processed feature
        ### if frame number condition is met, we can process the recheck model
        if (self.last_proc_frame - self.frame_id)==1:
            recheck_outputs = self.get_recheck_map(raw_id_feature, self.last_proc_feature, orig_feat)
            dets = self.fuse_recheck(recheck_outputs, dets)
            self.last_proc_frame = self.frame_id
            self.last_proc_feature = id_feature
