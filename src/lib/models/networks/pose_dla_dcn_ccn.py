...
class DLASegCCN(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
                 last_level, head_conv, out_channel=0):
        super(DLASegCCN, self).__init__()
        assert down_ratio in [2, 4, 8, 16]
        self.first_level = int(np.log2(down_ratio))
        self.last_level = last_level
        self.base = globals()[base_name](pretrained=pretrained)
        ...
        self.ccn = CCN(3, channels[self.first_level])
    ...
    def forward(self, x):
        ...
        det_inp, emb_inp = self.ccn(y[-1])

        z = {}
        for head in self.heads:
            if head == 'id':
                z[head] = self.__getattr__(head)(emb_inp)
            else:
                z[head] = self.__getattr__(head)(det_inp)
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
  model = DLASegCCN('dla{}'.format(num_layers), heads,
                 pretrained=True,
